...
```
cli/
├── keyword_search_cli.py          # BM25 keyword search interface
├── semantic_search_cli.py         # Semantic search interface
├── hybrid_search_cli.py           # Hybrid search interface
├── augmented_generation_cli.py    # RAG pipeline interface
├── evaluation_cli.py              # Search evaluation tools
├── lib/
│   ├── inverted_index.py          # BM25 inverted index implementation
│   ├── semantic_search.py         # Semantic search using embeddings
│   ├── chunked_semantic_search.py # Chunked semantic search
│   ├── hybrid_search.py           # Hybrid search algorithms
│   ├── query_enhancer.py          # LLM-based query enhancement
│   ├── llm_reranker.py            # LLM-based result re-ranking
│   ├── augment_generation.py      # RAG response generation
│   └── utils/                     # Utility functions
data/
├── movies.json                    # Movie dataset
├── golden_dataset.json            # Evaluation dataset
└── stopwords.txt                  # Stopwords for filtering
cache/                             # Generated embeddings and indexes
```
